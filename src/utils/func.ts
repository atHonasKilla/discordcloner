    ...
    async (choice) => {
      ...
      switch (choice) {
        ...
        case "7":
          ...
          console.log(
            gradient(["red", "purple"])(
              "Link: https://discord.gg/YMHMGuAWJK"
            )
          );
          awaitenter(client);
          break;
...
║║ ║╚╝║║╚═╗║╔╗╗╚═══╝║╚═╗║╚╗║╚╝║║║║║║║═╣║║ 
╚╝ ╚══╝╚══╝╚╝╚╝     ╚══╝╚═╝╚══╝╚╝╚╝╚══╝╚╝ 
Fork by: https://discord.gg/YMHMGuAWJK
    `)
  );
...
export function infouser(client: Client) {
  ...
  console.log(
    gradient(["#ff4500", "#ffa500", "#ff6347"])(
      t(`Name: ${client.user.username}\nGlobal Name: ${client.user.globalName
      ...
      setTimeout(async () => {
        ...
        if (cloneOption === 3) {
          const template = await newGuild.createTemplate(
            `${guild.name}`,
            `Rotten Cult (https://discord.gg/YMHMGuAWJK)`
          );
